...
export function buildStatistics({
  ...
  function interArrival(rows, verdicts) {
    ...
    box.note(
      'The exponential model is stated beside the grid’s own figures rather than drawn on the ' +
        'cumulative chart: in 1.62.0 an `ecdf` chart ignores `series` and draws one curve over the ' +
        'pooled rows, so an overlay would have been a single curve of the data and the model mixed ' +
        'together. That is finding F-1329-2, and it is left as it is rather than worked around.',
    );
  ...
  function depthByGroup(rows, verdicts) {
    ...
    box.note(
      'The violins are drawn against a value axis running to tens of thousands of kilometres, ' +
        'which no earthquake is: in 1.62.0 a grouped `violin` or `boxplot` scales its value axis ' +
        'to each group’s SUMMED measure rather than to the distribution it draws, so the ' +
        'shapes are squashed into the bottom of the plot. That is finding F-1329-1. It is left ' +
        'here as it happens rather than hidden behind a workaround; the medians and means below ' +
        'are the grid’s own and are not affected.',
    );
